...
public class DataInsert extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        ResultSet list = this.queries.getByModel(model);
 
       try {
           if(!list.next()){
               this.item.setModel(model);
               this.item.setDes(des);
               this.item.setName(name);
               this.item.setCount(1);
               this.item.setPrice(price);
               this.item.setTotal(price);
               this.item.setImage(image_path);
        
               int res = queries.insertToTable(item);
